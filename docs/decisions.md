...
## D61 — Compatibilidad del MCP con los conectores de chats online
Ajustes del authorization server (D56) contra los requisitos publicados de los conectores remotos (Claude.ai/Claude Code y clientes MCP en general): (1) las redirect_uri de loopback (`http://localhost|127.0.0.1`) se comparan ignorando el puerto (RFC 8252 §7.3) en authorize/approve — los clientes nativos abren un puerto efímero distinto por sesión y el match exacto rompía el flujo; el canje del code mantiene el match exacto contra lo usado en authorize. (2) Aliases de discovery que los SDKs prueban como fallback: `/.well-known/oauth-authorization-server/mcp` y `/.well-known/openid-configuration` sirven la misma metadata RFC 8414. (3) `offline_access` en `scopes_supported`: los conectores solo conservan el refresh token si el servidor lo anuncia y lo devuelve en el scope concedido; el refresh omite `scope` (RFC 6749 §5.1 = idéntico al concedido). (4) Revocación RFC 7009: `POST /oauth/revoke` (form `token`, siempre 200) borra el hash del token en `oauth_tokens`; anunciado como `revocation_endpoint`. Sin CORS específico para `/mcp`: los chats online conectan servidor-a-servidor. CIMD (client_id como URL de metadata) queda como pendiente si el DCR se vuelve ruidoso. Estado: firme.
...
## D63 — Tools del MCP en snake_case y conexión guiada por cliente
El separador `.` de D28 rompía la conexión real: los clientes de Claude (y otros) validan los nombres de tool contra `^[a-zA-Z0-9_-]{1,64}$`, así que un conector completaba el OAuth pero las tools con punto fallaban al listarse o llamarse. Todas las tools pasan a `<categoria>_<accion>` (`games_summary`, `music_top_artists`, `profile_search`…); los resources `ethos://…` no cambian (no son nombres de tool) y el campo `namespace` de los contextos y los previews de la web se alinean (`games_*`). Además, Conectar IA deja el toggle simulado: estado real vía `GET /mcp-status` (¿hay access token OAuth vigente del usuario? ¿token legacy emitido?) con botón "Comprobar conexión", guía en dos pasos (copiar URL → autorizar) con pestañas por cliente — Claude con los menús exactos (Ajustes → Conectores → Añadir conector personalizado), Claude Code con `claude mcp add --transport http` copiable, Cursor con el JSON de `mcpServers`, y "Otros" — presentando OAuth como vía principal; el token manual queda como "Avanzado" para clientes sin OAuth. Motivo: los pasos anteriores ("pega endpoint y token") no correspondían a cómo conectan los clientes actuales, que usan OAuth sin token. Estado: firme.

## D62 — Catálogo de proveedores alternativos integrado; GOG eliminado
Se integran los proveedores alternativos de D27 tras verificar su viabilidad (investigación 2026-07-11). Por API con username público: Last.fm (requiere `LASTFM_API_KEY` de la app; error 6/17 → estado `private`), MyAnimeList (header `X-MAL-CLIENT-ID` con `MAL_CLIENT_ID`; lista pública), Kitsu (JSON:API abierta, sin key) y Open Library (reading log público por username). Por API con token del usuario: Hardcover (GraphQL `me { user_books }`; el token se pega desde Settings → Hardcover API, caduca cada año y se guarda cifrado, D20). Por import con autodetección (amplía D49): StoryGraph (CSV), Letterboxd (diary/watched/ratings CSV — export gratis; los tres archivos del mismo export se combinan por obra), IMDb (ratings CSV) y, en música, Spotify (JSON del historial ampliado, formatos simple y extendido, descarta <30 s) y Apple Music (`Play History Daily Tracks.csv`, agregado por día). Los imports de música se combinan por (timestamp, artista, pista) con tope `MAX_IMPORTED_EVENTS = 10000` y el límite de cuerpo de imports sube a 25 MB. Fontanería multi-proveedor: `SourceStatus` gana `provider`/`mode` (persisten en `source_state` y viajan en `GET /sources/<cat>` y los contextos); una sola fuente activa por categoría (D4) — conectar borra la credencial de los hermanos y el primer refresco/import reemplaza los datos conservando las entradas a mano (D51); D5 se cubre como aviso de reemplazo con confirmación visual (el archivado del histórico queda pendiente). Web: selector de proveedor por categoría (ConnectHub) con guía paso a paso enlazada por proveedor y alta según tipo (username/token/import). **GOG se elimina** de docs y landing: sin API pública ni export autoservicio (solo ticket GDPR de semanas). Xbox (OpenXBL con key propia del usuario, sin minutos jugados fiables), PlayStation (NPSSO, no oficial, caduca ~2 meses) y TMDB (3-legged, solo ratings+watchlist) son conectables pero se difieren a la siguiente tarea; visibles como "próximamente" en el selector. Estado: firme.
